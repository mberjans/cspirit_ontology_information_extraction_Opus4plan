...
"""
    ...
    def test_basic_triple_extraction(
        self, file_path: str, file_type: str
    ) -> Dict[str, Any]:
        """Test basic triple extraction from a file."""
        logger.info(
            f"Testing basic triple extraction from {file_type} file: {file_path}"
        )
        ...
        try:
            ...
            logger.info(
                f"Successfully extracted {len(triples)} triples in {processing_time:.3f}s"
            )
    ...
    def test_parse_with_auto_extraction(
        self, file_path: str, file_type: str
    ) -> Dict[str, Any]:
        """Test parsing with automatic triple extraction enabled."""
        logger.info(
            f"Testing parse with auto-extraction from {file_type} file: {file_path}"
        )
        ...
        try:
            ...
            if "triples" in parsed_result:
                ...
                logger.info(
                    f"Auto-extraction produced {len(triples)} triples in {processing_time:.3f}s"
                )
            else:
    ...
    def test_triple_content_validation(
            ...
            try:
                ...
                if pred:
                    ...
                    validation_result["predicate_distribution"][pred_name] = (
                        validation_result["predicate_distribution"].get(pred_name, 0)
                        + 1
                    )
                ...
                validation_result["issues"].append(
                    f"Error validating triple {i}: {str(e)}"
                )

        # Calculate average confidence
        if len(triples) > 0:
            validation_result["confidence_stats"]["avg"] = total_confidence / len(
                triples
            )
...
"""
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"Error handling test: extracted {len(triples)} triples despite malformed input"
                )
            except Exception as e:
            ...
            if os.path.exists(file_path):
                try:
                    ...
                    result = {
                        ...
                        "triple_count": len(triples),
                        "processing_time": end_time - start_time,
                        "triples_per_second": len(triples) / (end_time - start_time)
                        if (end_time - start_time) > 0
                        else 0,
                    }

                    logger.info(
                        f"Performance test {os.path.basename(file_path)}: "
    ...
    def run_all_tests(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for file_type, file_path in sample_files.items():
                ...
                if result["success"]:
                    ...
                    if result["triples"]:
                        validation = self.test_triple_content_validation(
                            result["triples"]
                        )
        ...
        all_results["test_summary"]["total_time"] = (
            all_results["test_summary"]["end_time"]
            - all_results["test_summary"]["start_time"]
        )
    ...
    def print_results(self, results: Dict[str, Any]):
        ...
        for test in results["basic_extraction_tests"]:
            ...
            print(
                f"  {status} {test['file_type']}: {test['triple_count']} triples "
        ...
        for test in results["auto_extraction_tests"]:
            ...
            print(
                f"  {status} {test['file_type']}: {count} triples auto-extracted "
                f"({test['processing_time']:.3f}s)"
            )
        ...
        for validation in results["validation_results"]:
            print(
                f"  {validation['source_file']}: {validation['valid_count']}/{validation['total_count']} valid"
            )
        ...
        if perf_tests:
            ...
            for test in perf_tests:
                if test["success"]:
                    print(
                        f"  ✓ {test['file']}: {test['triple_count']} triples "
