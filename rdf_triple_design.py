...
class RDFTriple:
    ...
    def validate(self) -> bool:
        ...
        if self.object_type == "uri" and not self._is_valid_uri_or_identifier(self.object):
            self._validation_errors.append(f"Invalid object URI: {self.object}")
            
        # Validate blank nodes
        if self.object_type == "blank_node" and not self._is_valid_blank_node(self.object):
    ...
    def to_ntriples(self) -> str:
        ...
        subject_nt = format_uri(self.subject) if not self.subject.startswith("_:") else self.subject
        ...
        try:
            ...
            for cls in owl_ontology.classes():
                ...
                for parent in cls.is_a:
                    parent_uri = str(parent.iri) if hasattr(parent, 'iri') else str(parent)
                    triples.append(RDFTriple(
                        subject=cls_uri,
            ...
            raise OntologyException(
                ...
                error_code="AIM2_ONTO_PARS_E_002",
                context={
                    "ontology_name": str(owl_ontology.name) if hasattr(owl_ontology, 'name') else "unknown",
                    "extraction_method": "owlready2"
                }
            )
...
def extract_triples_method_design():
    ...
    def extract_triples(self, parsed_result: Any, **kwargs) -> List[RDFTriple]:
        ...
        include_inferred = kwargs.get("include_inferred_triples", False)
        ...
        if extractor_preference in ("rdflib", "both") and self._rdflib_available:
            try:
                ...
                self.logger.debug(f"Extracted {len(rdflib_triples)} triples using rdflib")
            except Exception as e:
        ...
        if extractor_preference in ("owlready2", "both") and self._owlready2_available:
            try:
                ...
                self.logger.debug(f"Extracted {len(owlready2_triples)} triples using owlready2")
            except Exception as e:
