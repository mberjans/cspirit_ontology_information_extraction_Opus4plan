...
<rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"
    ...
    try:
        ...
        parser.set_options(
            {"continue_on_error": True, "error_recovery": True, "timeout_seconds": 30}
        )
        ...
        if triples:
            ...
            found_subjects = sum(
                1
                for t in triples
                if any(subj in t.subject for subj in expected_subjects)
            )
            found_predicates = sum(
                1
                for t in triples
                if any(pred in t.predicate for pred in expected_predicates)
            )
    ...
    for file_path in test_files:
        if os.path.exists(file_path):
            try:
                ...
                print(
                    f"  ✓ Extracted {len(triples)} triples in {end_time - start_time:.3f}s"
                )
                print(
                    f"  - Performance: {len(triples)/(end_time - start_time):.1f} triples/sec"
                )

                if triples:
                    print(
                        f"  - Sample triple: {triples[0].subject} -> {triples[0].predicate} -> {triples[0].object}"
                    )
