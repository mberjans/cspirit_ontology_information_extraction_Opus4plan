...
class ProgressIndicator:
    ...
    def update(self):
        ...
        if not self.verbose and self.running:
            print(
                f"\r{Colors.CYAN}▶ {self.message} {self.spinner_chars[self.current_char]}{Colors.RESET}",
                end="",
                flush=True,
            )
    ...
    def finish(self, success: bool = True, message: Optional[str] = None):
        ...
        if self.running:
            ...
            if not self.verbose:
                print(
                    f"\r{status_color}{status_char} {self.message} - {final_message}{Colors.RESET}"
                )
            else:
...
class VirtualEnvSetup:
    ...
    def print_banner(self):
        ...
        print(
            f"{Colors.CYAN}Setup Mode:{Colors.RESET} {'Development' if not self.args.prod else 'Production'}"
        )
        print(f"{Colors.CYAN}Python Executable:{Colors.RESET} {self.python_executable}")
        print(
            f"{Colors.CYAN}Platform:{Colors.RESET} {self.system_info['platform']} ({self.system_info['architecture']})"
        )
        print()
    ...
    def check_python_version(self) -> bool:
        ...
        try:
            ...
            if current_version < required_version:
                progress.finish(
                    False,
                    f"Python {version_str} < {'.'.join(map(str, required_version))}",
                )
                print(
                    f"{Colors.RED}✗ Error: Python {'.'.join(map(str, required_version))}+ is required"
                )
    ...
    def remove_existing_venv(self) -> bool:
        ...
        progress = ProgressIndicator(
            "Removing existing virtual environment", self.verbose
        )
        progress.start()
    ...
    def create_virtual_environment(self) -> bool:
        ...
        if self.check_existing_venv() and not self.args.force_recreate:
            print(
                f"{Colors.GREEN}✓ Virtual environment already exists: {self.venv_path}{Colors.RESET}"
            )
            return True
    ...
    def upgrade_pip(self) -> bool:
        ...
        try:
            ...
            if result.returncode != 0:
                error_msg = (
                    result.stderr if hasattr(result, "stderr") else "Unknown error"
                )
    ...
    def install_requirements(self, requirements_file: Path, description: str) -> bool:
        ...
        if not requirements_file.exists():
            print(
                f"{Colors.YELLOW}⚠ Warning: {requirements_file} not found, skipping{Colors.RESET}"
            )
            return True
    ...
    def install_project_package(self) -> bool:
        ...
        progress = ProgressIndicator(
            "Installing project package (editable)", self.verbose
        )
        progress.start()
        ...
        try:
            ...
            for package in test_imports:
                ...
                if result.returncode != 0:
                    ...
                    if self.verbose:
                        print(
                            f"{Colors.RED}Import error for {package}:\n{result.stderr}{Colors.RESET}"
                        )
                    return False
    ...
    def print_activation_instructions(self):
        ...
        print(f"  {Colors.CYAN}Virtual Environment:{Colors.RESET} {self.venv_path}")
        print(
            f"  {Colors.CYAN}Python Executable:{Colors.RESET} {self.get_venv_python()}"
        )
        print(
            f"  {Colors.CYAN}Configuration:{Colors.RESET} {self.project_root}/aim2_project/configs/"
        )
    ...
    def run_setup(self) -> bool:
        ...
        if not self.args.prod:
            if not self.install_requirements(
                self.dev_requirements_file, "development dependencies"
            ):
                return False
        ...
        if not self.verify_installation():
            print(
                f"{Colors.YELLOW}⚠ Warning: Installation verification failed, but setup may still be usable{Colors.RESET}"
            )
...
Examples:
  ...
  python setup_env.py --verbose          # Enable verbose output
    ...
    parser.add_argument(
        "--prod", action="store_true", help="Install production dependencies only"
    )
    ...
    parser.add_argument("--python", help="Specify Python executable path")
