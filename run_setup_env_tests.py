...
class SetupEnvTestRunner:
    ...
    def run_tests(
        ...
        if coverage:
            cmd.extend(
                ["--cov=setup_env", "--cov-report=term-missing", "--cov-report=xml"]
            )
        ...
        if html_report:
            cmd.extend(
                ["--html=test_reports/setup_env_report.html", "--self-contained-html"]
            )
...
def main():
    ...
    try:
        ...
        if exit_code == 0:
            ...
            if args.html_report:
                html_report_path = (
                    project_root / "test_reports" / "setup_env_report.html"
                )
