        ...
        with patch(
            ...
            for i, file_path in enumerate(valid_files):
                if file_path.endswith(".owl"):
                    ...
                    mock_parsers.append(
                        create_mock_parser("jsonld", sample_ontologies[2])
                    )
            ...
            if len(successful_results) != len(valid_files):
                logger.error(
                    f"Expected {len(valid_files)} successful loads, got {len(successful_results)}"
                )
                for result in failed_results:
                    logger.error(
                        f"Failed to load {result.source_path}: {result.errors}"
                    )
                return False
            ...
            if len(manager.ontologies) != len(successful_results):
                logger.error(
                    f"Expected {len(successful_results)} stored ontologies, got {len(manager.ontologies)}"
                )
                return False
    ...
    try:
        ...
        with patch(
            ...
            if len(successful_results) > 0:
                logger.error(
                    f"Expected no successful loads, got {len(successful_results)}"
                )
                return False

            if len(failed_results) != len(invalid_files):
                logger.error(
                    f"Expected {len(invalid_files)} failed loads, got {len(failed_results)}"
                )
                return False
...
def test_ontology_storage_and_access(manager: OntologyManager) -> bool:
    ...
    try:
        ...
        if not ontology_ids:
            logger.warning(
                "No ontologies found - this may be expected if previous tests used mocks"
            )
            return True
        ...
        for ontology_id in ontology_ids:
            ...
            logger.info(
                f"✓ Successfully accessed ontology {ontology_id}: {ontology.name}"
            )
...
def test_statistics_and_reporting(manager: OntologyManager) -> bool:
    ...
    try:
        ...
        if stats["total_loads"] != stats["successful_loads"] + stats["failed_loads"]:
            logger.error(
                "Statistics inconsistency: total_loads != successful_loads + failed_loads"
            )
            return False
        ...
        with patch(
            ...
            if manager.enable_caching and manager.cache_size_limit > 0:
                if mock_parser.parse.call_count == 1:
                    logger.info(
                        "✓ Cache efficiency confirmed - parser called only once"
                    )
                else:
                    logger.warning(
                        f"Parser called {mock_parser.parse.call_count} times - cache may not be working"
                    )
...
def main():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            valid_files = [
                path
                for key, (path, ont) in test_files.items()
            ...
            results["multi_source_loading"] = test_multi_source_loading(
                manager, valid_files
            )

            # Test 2: Error handling
            manager_for_errors = OntologyManager(
                enable_caching=True, cache_size_limit=10
            )
            results["error_handling"] = test_error_handling(
                manager_for_errors, invalid_files
            )
            ...
            with patch(
                ...
                logger.info(
                    f"Mixed scenario: {len(successful_mixed)} successful, {len(failed_mixed)} failed"
                )
                results["mixed_scenario"] = len(successful_mixed) == len(
                    valid_files
                ) and len(failed_mixed) == len(invalid_files)
