...
class TestSetupIntegration:
    ...
    def test_package_builds_successfully(self, isolated_environment):
        ...
        ], cwd=isolated_environment, capture_output=True, text=True)
    ...
    def test_package_installs_in_clean_environment(self, virtual_environment):
        ...
        ], cwd=virtual_environment["project_dir"], capture_output=True, text=True)
        ...
        import_test = subprocess.run([
            virtual_environment["python"], "-c", "import aim2_project; print('Import successful')"
    ...
    def test_dependencies_install_correctly(self, virtual_environment):
        ...
        for dep in key_dependencies:
            import_test = subprocess.run([
                virtual_environment["python"], "-c", f"import {dep}; print('{dep} available')"
            ], capture_output=True, text=True)
    ...
    def test_console_scripts_work(self, virtual_environment):
        ...
        for script in expected_scripts:
            ...
            if sys.platform == "win32":
                script_path = virtual_environment["venv_dir"] / "Scripts" / f"{script}.exe"
            else:
            ...
            if script_path.exists():
                ...
                assert result.returncode in [0, 1, 2]  # Allow for not-implemented scripts
...
class TestSetupWithRealDependencies:
    ...
    def test_core_dependencies_resolve(self, clean_venv):
        ...
        for dep in test_dependencies:
            ...
            if import_result.returncode != 0:
                pytest.fail(f"Failed to import {import_name} (from package {package_name}): {import_result.stderr}")
    ...
    def test_ml_dependencies_resolve(self, clean_venv):
        ...
        for dep in ml_dependencies:
            result = subprocess.run([
                clean_venv["pip"], "install", dep, "--index-url", "https://download.pytorch.org/whl/cpu"
            ], capture_output=True, text=True, timeout=300)  # 5 minute timeout
            
            if result.returncode != 0:
                pytest.skip(f"Skipping {dep} due to installation issues: {result.stderr}")
    ...
    def test_python_version_compatibility(self):
        ...
        if setup_py.exists():
            try:
                ...
                pytest.fail(f"setup.py syntax incompatible with Python {current_version}: {e}")
