    ...
    def test_no_version_conflicts_in_core_dependencies(self):
        ...
        for dep_str in dependencies:
            ...
            if req.name in package_versions:
                ...
                assert existing == current, f"Version conflict for {req.name}: {existing} vs {current}"
            else:
...
class TestDependencyInstallation:
    ...
    def test_dependency_groups_are_properly_separated(self):
        ...
        dependency_groups = {
            "core": [
                "numpy", "pandas", "scikit-learn", "spacy", "transformers",
                "torch", "rdflib", "owlready2", "networkx", "pyyaml",
                "click", "tqdm", "requests"
            ],
            "dev": [
