...
class TestProgressInfo(unittest.TestCase):
    ...
    def test_progress_info_percentage_calculation(self):
        ...
        # Normal case
        info = ProgressInfo(
            phase=ProgressPhase.PARSING, current_step=25, total_steps=100
        )
        self.assertEqual(info.percentage, 25.0)

        # Zero total steps
        info_zero = ProgressInfo(
            phase=ProgressPhase.PARSING, current_step=5, total_steps=0
        )
        self.assertEqual(info_zero.percentage, 0.0)

        # Complete case
        info_complete = ProgressInfo(
            phase=ProgressPhase.COMPLETED, current_step=10, total_steps=10
        )
    ...
    def test_progress_info_immutability(self):
        ...
        info = ProgressInfo(phase=ProgressPhase.PARSING, current_step=5, total_steps=10)
...
class TestProgressReporter(unittest.TestCase):
    ...
    def test_report_progress_callback_error_handling(self):
        ...
        progress_info = ProgressInfo(
            phase=ProgressPhase.PARSING, current_step=1, total_steps=5
        )
    ...
    def test_reset(self):
        ...
        progress_info = ProgressInfo(
            phase=ProgressPhase.PARSING, current_step=5, total_steps=10
        )
        self.reporter.report_progress(progress_info)
        self.reporter.cancel()
...
class TestAbstractParserProgress(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with patch("aim2_project.aim2_ontology.parsers.ConfigManager") as mock_config:
            ...
            self.parser = TestParser(parser_name="test_parser")
    ...
    def test_report_progress_when_disabled(self):
        ...
        self.parser._report_progress(ProgressPhase.PARSING, 1, 5, "Test message")
    ...
    def test_get_current_progress(self):
        ...
        progress_info = ProgressInfo(
            phase=ProgressPhase.PARSING, current_step=2, total_steps=5
        )
        ...
        class SimulatedParser(AbstractParser):
            def parse(self, content: str, **kwargs) -> Any:
                ...
                self._report_progress(
                    ProgressPhase.INITIALIZING, 1, 5, "Starting parse"
                )
                self._report_progress(
                    ProgressPhase.VALIDATING, 2, 5, "Validating input"
                )
                self._report_progress(ProgressPhase.PARSING, 3, 5, "Parsing content")
                self._report_progress(
                    ProgressPhase.BUILDING_ONTOLOGY, 4, 5, "Building result"
                )
...
class TestProgressThreadSafety(unittest.TestCase):
    ...
    def test_concurrent_progress_reporting(self):
        ...
        for thread_id in range(num_threads):
            thread = threading.Thread(target=report_progress_worker, args=(thread_id,))
        ...
        # Check results
        self.assertEqual(
            len(self.callback_errors), 0, "No callback errors should occur"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            len(thread_ids), num_threads, "All threads should have reported"
        )
    ...
    def test_concurrent_cancellation(self):
        ...
        final_states = [
            result for state, result in cancellation_results if state == "final"
        ]
...
class TestProgressPerformance(unittest.TestCase):
    ...
    def test_progress_reporting_overhead(self):
        ...
        for i in range(num_reports):
            progress_info = ProgressInfo(
                phase=ProgressPhase.PARSING, current_step=i, total_steps=num_reports
            )
            self.reporter.report_progress(progress_info)
