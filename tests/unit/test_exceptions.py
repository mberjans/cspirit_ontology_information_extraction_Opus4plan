...
# Mock exception classes for testing - these would be the actual implementations
class AIM2Exception(Exception):
    """
    Base exception class for AIM2 project with error code system.
    
    This is the base exception that all other AIM2 exceptions inherit from.
    It provides error code management, serialization, and cause tracking.
    ...
    def to_dict(self) -> Dict[str, Any]:
        """
        Serialize exception to dictionary format.
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'AIM2Exception':
        ...
        Args:
            data: Dictionary containing exception information
            
        Returns:
            AIM2Exception instance
        """
        cause = None
        if data.get("cause_type") and data.get("cause_message"):
            # Create a generic exception for the cause
...
class OntologyException(AIM2Exception):
    ...
    Raised when operations involving ontology management, loading, parsing,
    or validation encounter errors.
    """
...
class ExtractionException(AIM2Exception):
    """
    Exception for information extraction errors.
    
    Raised when text processing, NER, relationship extraction, or other
    information extraction operations encounter errors.
    """
...
class LLMException(AIM2Exception):
    ...
    Raised when LLM API calls, model interactions, or LLM-related
...
class ValidationException(AIM2Exception):
    ...
    Raised when data validation, schema validation, or configuration
    validation operations encounter errors.
    """
...
class TestExceptionChaining:
    ...
    def test_nested_exception_chaining(self):
        ...
        top_error = ExtractionException("Extraction pipeline failed", cause=middle_error)
