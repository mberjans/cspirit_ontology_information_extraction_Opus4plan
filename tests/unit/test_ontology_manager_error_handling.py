...
class TestOntologyManagerErrorHandling:
    ...
    def test_load_nonexistent_file(self, ontology_manager):
        ...
        with patch(
            ...
            mock_auto_detect.return_value = (
                None  # Parser not found for nonexistent file
            )
    ...
    def test_load_permission_denied_file(self, ontology_manager, temp_dir):
        ...
        try:
            with patch(
                ...
                assert len(result.errors) > 0
                assert any(
                    "unexpected error" in error.lower() for error in result.errors
                )
    ...
    def test_malformed_owl_file(self, ontology_manager, temp_dir):
        ...
        malformed_owl_cases = [
            ...
            (
                "missing_namespace.owl",
                "<owl:Ontology>missing namespaces</owl:Ontology>",
            ),
            ("invalid_structure.owl", "<?xml version='1.0'?><not_owl_content/>"),
            (
                "corrupted_encoding.owl",
                "<?xml version='1.0' encoding='utf-8'?>\xFF\xFE<owl:Ontology/>",
            ),
        ]
        ...
        for filename, content in malformed_owl_cases:
            ...
            with patch(
                ...
                assert (
                    len(result.errors) > 1
                )  # Should include both manager and parser errors
                assert any(
                    "failed to parse ontology" in error.lower()
    ...
    def test_malformed_jsonld_file(self, ontology_manager, temp_dir):
        ...
        malformed_jsonld_cases = [
            ...
            (
                "invalid_ld_structure.jsonld",
                '{"@context": [], "@graph": "not an array"}',
            ),
        ]
    ...
    def test_invalid_ontology_structure(self, ontology_manager, temp_dir):
        ...
        with patch(
            ...
            assert len(result.errors) > 0
            assert any(
                "invalid ontology type" in error.lower() for error in result.errors
            )
    ...
    def test_null_none_inputs(self, ontology_manager):
        ...
        for invalid_input in none_inputs:
            ...
            if invalid_input is None:
                assert any(
                    "either file_path or content must be provided" in error.lower()
                    for error in result.errors
                )
            else:
                assert any(
                    "no suitable parser" in error.lower()
                    or "unexpected error" in error.lower()
                    for error in result.errors
                )
    ...
    def test_cache_corruption_scenarios(self, ontology_manager, sample_ontology):
        ...
        with patch(
            ...
            assert (
                "access_count" in result.errors[0]
            )  # Specific error from cache corruption

    def test_concurrent_access_errors(
        self, ontology_manager, sample_ontology, temp_dir
    ):
        ...
        assert len(errors) == 0 or all(
            isinstance(e, (RuntimeError, AttributeError)) for e in errors
        )
        assert len(results) >= 1  # At least some should succeed

    def test_state_inconsistency_after_partial_failures(
    ...
    def test_recovery_from_failed_operations(
        self, ontology_manager, temp_dir, sample_ontology
    ):
    ...
    def test_exception_chaining_and_propagation(self, ontology_manager, temp_dir):
        ...
        with patch(
            ...
            chained_error = RuntimeError(
                "Parser wrapper error with original: Original parsing error"
            )
    ...
    def test_logging_integration_with_error_handling(self, ontology_manager, temp_dir):
        ...
        with patch(
            ...
            with patch.object(ontology_manager.logger, "error") as mock_error_log:
                with patch.object(
                    ontology_manager.logger, "exception"
                    ...
                    if mock_exception_log.called:
                        ...
                        assert any(
                            "unexpected error" in str(arg).lower()
                            for arg in log_call_args
                        )
    ...
    def test_multiple_error_types_in_sequence(self, ontology_manager, temp_dir):
        ...
        with patch(
            ...
            mock_auto_detect.side_effect = [
                error_parser_factory(error) for error in error_types
            ]
    ...
    def test_error_handling_with_caching_enabled_and_disabled(
        ...
        for enable_caching, description in error_scenarios:
            ...
            with patch(
                ...
                ), f"Cache should be empty after error {description}"
