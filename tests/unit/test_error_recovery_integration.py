...
class TestErrorRecoveryWorkflow:
    ...
    def test_complete_error_recovery_workflow_syntax_error(self, mock_parser_with_recovery):
    ...
    def test_complete_error_recovery_workflow_fatal_error(self, mock_parser_with_recovery):
    ...
    def test_multiple_error_recovery_workflow(self, mock_parser_with_recovery):
        ...
        assert stats.recoverable_errors == 4  # All these errors are classified as recoverable
...
class TestParserErrorRecoveryIntegration:
    ...
    def owl_parser_with_recovery(self):
        ...
        def mock_parse_with_recovery(content, **kwargs):
            ...
            if "malformed" in content.lower():
                ...
                return {"status": "recovered", "content": content.replace("malformed", "corrected")}
            else:
    ...
    def csv_parser_with_recovery(self):
        ...
        def mock_parse_with_recovery(content, **kwargs):
            ...
            return {"status": "recovered" if parser.error_stats.total_errors > 0 else "success", 
    ...
    def configurable_parser(self):
        ...
        def mock_parse_with_config(content, **kwargs):
            ...
            if "syntax_error" in content:
                ...
                strategy = parser.config["error_recovery"]["recovery_strategies"]["SyntaxError"]
                
                parser.error_stats.total_errors += 1
                parser.error_stats.recoverable_errors += 1
                
                if strategy == "skip" and parser.config["error_recovery"]["skip_malformed_rows"]:
                    # Skip this content
                    parser.error_stats.successful_recoveries += 1
                    errors_encountered.append({"error": "SyntaxError", "action": "skipped"})
                
            elif "value_error" in content:
                error = ValueError("Value error")
                strategy = parser.config["error_recovery"]["recovery_strategies"]["ValueError"]
                
                parser.error_stats.total_errors += 1
                parser.error_stats.recoverable_errors += 1
                
                if strategy == "default" and parser.config["error_recovery"]["use_default_values"]:
                    # Use default value
                    parser.error_stats.successful_recoveries += 1
                    results.append({"field": "default_value"})
                    errors_encountered.append({"error": "ValueError", "action": "default_used"})
            
            elif "key_error" in content:
                error = KeyError("Key error")
                strategy = parser.config["error_recovery"]["recovery_strategies"]["KeyError"]
                ...
                if strategy == "continue":
                    ...
                    errors_encountered.append({"error": "KeyError", "action": "continued"})
...
class TestErrorRecoveryPerformance:
    ...
    def test_error_recovery_scalability(self, performance_parser):
        ...
        test_cases = [
            ("normal " * i, False) for i in range(1, 6)  # Normal cases
        ] + [
        ...
        avg_processing_time = sum(performance_parser.processing_times) / len(performance_parser.processing_times)
...
class TestErrorRecoveryReporting:
    ...
    def test_comprehensive_error_reporting(self, reporting_parser):
        ...
        # Check warning entry
        warning_entry = next(entry for entry in detailed_log if entry["severity"] == "WARNING")
