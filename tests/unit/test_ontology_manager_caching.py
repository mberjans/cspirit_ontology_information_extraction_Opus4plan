...
<rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"
    ...
    def test_cache_disabled_functionality(self, ontology_manager_no_cache, create_test_file):
        ...
        with patch("aim2_project.aim2_ontology.ontology_manager.auto_detect_parser") as mock_auto_detect:
            ...
            # When caching is disabled, cache misses should not be tracked
            assert ontology_manager_no_cache.load_stats["cache_misses"] == 0
    ...
    def test_file_recreation_with_same_name(self, ontology_manager, create_test_file, temp_dir):
...
<rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"
    ...
    def test_cache_size_limit_zero(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            with patch("aim2_project.aim2_ontology.ontology_manager.auto_detect_parser") as mock_auto_detect:
                ...
                parser.parse.return_value = ParseResult(
                    success=True,
                    data=Ontology(id="TEST:001", name="Test", version="1.0", terms={}, relationships={}, namespaces=[]),
                    errors=[],
                    warnings=[],
                    metadata={},
                    parse_time=0.1,
    ...
    def test_cache_size_limit_one(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            ontology1 = Ontology(id="TEST:001", name="Test1", version="1.0", terms={}, relationships={}, namespaces=[])
            ontology2 = Ontology(id="TEST:002", name="Test2", version="1.0", terms={}, relationships={}, namespaces=[])
            
            with patch("aim2_project.aim2_ontology.ontology_manager.auto_detect_parser") as mock_auto_detect:
                parser1 = Mock()
                parser1.format_name = "owl"
                parser1.parse.return_value = ParseResult(
                    success=True, data=ontology1, errors=[], warnings=[], metadata={}, parse_time=0.1
                )
                
                parser2 = Mock()
                parser2.format_name = "owl"
                parser2.parse.return_value = ParseResult(
                    success=True, data=ontology2, errors=[], warnings=[], metadata={}, parse_time=0.1
                )
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            for i in range(4):
                ...
                ontologies.append(Ontology(
                    id=f"TEST:{i:03d}", name=f"Test{i}", version="1.0", 
                    terms={}, relationships={}, namespaces=[]
                ))
    ...
    def test_concurrent_cache_eviction(self, ontology_manager):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            for i in range(5):
                ...
                files.append(str(file_path))
                ontologies.append(Ontology(
                    id=f"CONCURRENT:{i:03d}", name=f"Concurrent{i}", version="1.0",
                    terms={}, relationships={}, namespaces=[]
                ))
            ...
            with patch("aim2_project.aim2_ontology.ontology_manager.auto_detect_parser") as mock_auto_detect:
                ...
                with ThreadPoolExecutor(max_workers=5) as executor:
                    futures = [executor.submit(concurrent_load_worker, i) for i in range(5)]
    ...
    def test_filesystem_errors_during_cache_operations(self, ontology_manager, create_test_file):
    ...
    def test_io_error_handling_during_cache_updates(self, ontology_manager, create_test_file):
            ...
            for i in range(3):
                ...
                for j in range(100):
                    ...
                    terms[term_id] = Term(
                        ...
                        definition=f"Long definition for memory test term {j}" * 20,  # Long definitions
                        synonyms=[f"synonym{j}_{k}" * 5 for k in range(10)],  # Many long synonyms
            ...
            with patch("aim2_project.aim2_ontology.ontology_manager.auto_detect_parser") as mock_auto_detect:
                def mock_parser_behavior(*args, **kwargs):
                    ...
                    if file_path.endswith("invalid.owl"):
                        ...
                        parser.format_name = "owl" 
                        parser.parse.return_value = ParseResult(
                            success=False, data=None, errors=["Invalid content"], warnings=[], metadata={}, parse_time=0.1
                        )
                        return parser
                        ...
                        parser.format_name = "owl"
                        parser.parse.return_value = ParseResult(
                            success=True, data=valid_ontology, errors=[], warnings=[], metadata={}, parse_time=0.1
                        )
                        return parser
                    else:
                        return None
    ...
    def test_cache_state_after_ontology_removal(self, ontology_manager, create_test_file):
