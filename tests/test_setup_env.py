...
class TestVirtualEnvSetupMethods:
    ...
    def test_check_existing_venv_exists(self, setup_instance):
        ...
        with patch.object(setup_instance.venv_path, "exists", return_value=True):
            with patch.object(
                setup_instance.venv_path / "pyvenv.cfg", "exists", return_value=True
            ):
    ...
    def test_create_virtual_environment_success(self, mock_subprocess, setup_instance):
        ...
        with patch.object(
            setup_instance, "check_existing_venv", side_effect=[False, True]
        ):
    ...
    def test_install_requirements_success(self, mock_sleep, mock_popen, setup_instance):
        ...
        with patch.object(requirements_file, "exists", return_value=True):
            result = setup_instance.install_requirements(
    ...
    def test_install_requirements_file_not_found(self, setup_instance):
        ...
        with patch.object(requirements_file, "exists", return_value=False):
            ...
            assert (
                result is True
            )  # Should succeed (with warning) when file doesn't exist
...
class TestVirtualEnvSetupIntegration:
    ...
    def temp_project_dir(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (aim2_dir / "requirements.txt").write_text(
                "requests>=2.25.0\npyyaml>=6.0\n"
            )
...
"""
    ...
    def test_complete_setup_workflow_dev_mode(
        self, temp_project_dir, mock_args_integration
    ):
                ...
                with patch("subprocess.run") as mock_subprocess:
                    with patch("subprocess.Popen") as mock_popen:
                        ...
                        setup.requirements_file = (
                            temp_project_dir / "aim2_project" / "requirements.txt"
                        )
                        setup.dev_requirements_file = (
                            temp_project_dir / "requirements-dev.txt"
                        )

                        # Mock venv existence checks
                        with patch.object(
                            setup, "check_existing_venv", return_value=False
                        ):
                            # Run the setup
    ...
    def test_complete_setup_workflow_prod_mode(
        self, temp_project_dir, mock_args_integration
    ):
...
class TestArgumentParsing:
    ...
    def test_parse_arguments_combined(self):
        ...
        with patch(
            "sys.argv",
            ["setup_env.py", "--prod", "--venv-name", "prod_env", "--verbose"],
        ):
            args = setup_env.parse_arguments()
...
class TestErrorHandling:
    ...
    @patch("subprocess.run")
    def test_network_timeout_during_pip_install(
        self, mock_subprocess, setup_with_mocked_args
    ):
    ...
    def test_unable_to_remove_existing_venv(self, mock_rmtree, setup_with_mocked_args):
        ...
        with patch.object(
            setup_with_mocked_args, "check_existing_venv", return_value=True
        ):
...
class TestCrossPlatformCompatibility:
    ...
    def test_activation_instructions_windows(self, setup_windows):
        ...
        with patch("builtins.print") as mock_print:
            ...
            assert (
                "Scripts\\activate.bat" in printed_text or "PowerShell" in printed_text
            )
...
class TestPerformanceAndTimeouts:
    ...
    def test_venv_creation_timeout(self, mock_subprocess, setup_with_timeouts):
        ...
        with patch.object(
            setup_with_timeouts, "check_existing_venv", return_value=False
        ):
            start_time = time.time()
...
class TestMainFunction:
    ...
    def test_main_function_success(self):
        ...
        with patch("sys.argv", ["setup_env.py"] + test_args):
            with patch.object(
                setup_env.VirtualEnvSetup, "run_setup", return_value=True
            ):
    ...
    def test_main_function_failure(self):
        ...
        with patch("sys.argv", ["setup_env.py"] + test_args):
            with patch.object(
                setup_env.VirtualEnvSetup, "run_setup", return_value=False
            ):
                with patch("sys.exit") as mock_exit:
                    setup_env.main()
    ...
    def test_main_function_keyboard_interrupt(self):
        ...
        with patch("sys.argv", ["setup_env.py"] + test_args):
            with patch.object(
                setup_env.VirtualEnvSetup, "run_setup", side_effect=KeyboardInterrupt
            ):
                with patch("sys.exit") as mock_exit:
                    setup_env.main()
