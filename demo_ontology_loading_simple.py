    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if single_result.success:
                logger.info(
                    f"✓ Successfully loaded single file: {single_result.ontology.name}"
                )
            ...
            for result in successful:
                logger.info(
                    f"  ✓ {Path(result.source_path).name}: {result.ontology.name if result.ontology else 'N/A'}"
                )
            ...
            for ont_id in ontology_ids:
                ...
                if ontology:
                    ...
                    logger.info(
                        f"    Terms: {len(ontology.terms)}, Relationships: {len(ontology.relationships)}"
                    )
                else:
            ...
            if successful:
                ...
                logger.info(
                    f"Re-loading {Path(first_successful_file).name} to test cache..."
                )
            ...
            if not error_result.success:
                ...
                logger.info(
                    f"  Error: {error_result.errors[0] if error_result.errors else 'No specific error'}"
                )
            else:
                logger.error(
                    "✗ Expected error for non-existent file, but load succeeded"
                )
