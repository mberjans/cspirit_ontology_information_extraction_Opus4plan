...
GO:0008152,metabolic process,The chemical reactions and pathways,biological_process"""
    ...
    def test_load_ontology_success(self, mock_auto_detect, ontology_manager, mock_parser, sample_ontology):
    ...
    @patch('aim2_project.aim2_ontology.ontology_manager.auto_detect_parser')
    def test_load_ontologies_multiple_sources(self, mock_auto_detect, ontology_manager, mock_parser):
        ...
        assert len(ontology_manager.ontologies) == 1  # Same ontology loaded 3 times (overwrites)
        assert ontology_manager.load_stats['total_loads'] == 3
        assert ontology_manager.load_stats['successful_loads'] == 3

    @patch('aim2_project.aim2_ontology.ontology_manager.auto_detect_parser')
    def test_caching_functionality(self, mock_auto_detect, ontology_manager, mock_parser, temp_dir):
        ...
        assert 'cache_hit' not in result1.metadata or result1.metadata['cache_hit'] is False
    ...
    @patch('aim2_project.aim2_ontology.ontology_manager.auto_detect_parser')
    def test_cache_invalidation_on_file_modification(self, mock_auto_detect, ontology_manager, mock_parser, temp_dir):
    ...
    @pytest.mark.parametrize("file_extension,expected_calls", [
        ("test.owl", 1),
        ("test.csv", 1),
        ("test.jsonld", 1),
        ("test.json", 1),
    ...
    def test_format_detection_coverage(self, ontology_manager, mock_parser, file_extension, expected_calls):
