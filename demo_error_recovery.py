...
class DemoParser:
    ...
    def _classify_error_severity(
        ...
        if (
            error_type in ["KeyError", "AttributeError"]
            and "optional" in str(error).lower()
        ):
    ...
    def _select_recovery_strategy(
        ...
        if (
            ...
            and self.config["error_recovery"]["abort_on_fatal"]
        ):
        ...
        if error_type in ["SyntaxError", "ValueError"]:
            if (
                RecoveryStrategy.SKIP not in attempted
                and self.config["error_recovery"]["skip_malformed_rows"]
            ):
                return RecoveryStrategy.SKIP
            elif (
                RecoveryStrategy.DEFAULT not in attempted
                and self.config["error_recovery"]["use_default_values"]
            ):
    ...
    def _apply_recovery_strategy(self, error_context: ErrorContext, **kwargs) -> Any:
        ...
        self.logger.info(
            f"Applying recovery strategy '{strategy.value}' for {type(error_context.error).__name__}"
        )

        if strategy == RecoveryStrategy.SKIP:
            self.logger.info(
                f"Skipping problematic content at {error_context.location}"
            )
            return None
            ...
            self.logger.error(
                f"Aborting due to unrecoverable error: {error_context.error}"
            )
    ...
    def _get_default_value(self, error_context: ErrorContext) -> Any:
        ...
        if self.parser_type == "OWL":
            ...
            return {"@context": {"@vocab": "http://schema.org/"}, "@type": "Thing"}
        else:
            return {}
    ...
    def _handle_parse_error(
        ...
        try:
            ...
            if (
                ...
                or error_context.recovery_strategy == RecoveryStrategy.SKIP
            ):
                self.error_stats.successful_recoveries += 1
                self.logger.info(
                    f"Successfully recovered from {type(error).__name__} using {error_context.recovery_strategy.value}"
                )
            else:
    ...
    def parse(self, content: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            error_context = self._handle_parse_error(
                e, location="main parsing", content=content
            )
            results["errors"].append(
                {
                    ...
                    "recovery_successful": error_context.recovery_data.get("result")
                    is not None,
                }
            )
...
def demonstrate_error_classification():
    ...
    for error, location in test_errors:
        ...
        print(
            f"Error: {type(error).__name__:<20} | Severity: {severity.value:<12} | Location: {location}"
        )
...
def demonstrate_recovery_strategy_selection():
    ...
    test_scenarios = [
        ...
        (
            KeyError("Missing optional field"),
    ...
    for error, severity, attempted, location in test_scenarios:
        ...
        print(
            f"Error: {type(error).__name__:<20} | Severity: {severity.value:<12} | "
...
def demonstrate_configuration_impact():
    ...
    print(
        f"  Skip Malformed: {parser_default.config['error_recovery']['skip_malformed_rows']}"
    )
    print(
        f"  Use Defaults: {parser_default.config['error_recovery']['use_default_values']}"
    )
    ...
    if result_default["errors"]:
        print(
            f"  Recovery Strategy: {result_default['errors'][0]['recovery_strategy']}"
        )
    ...
    if result_modified["errors"]:
        print(
            f"  Recovery Strategy: {result_modified['errors'][0]['recovery_strategy']}"
        )
